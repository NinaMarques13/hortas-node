...
// =====================================================
// POST /api/hortas — Cadastrar horta (requer autenticação)
// =====================================================
router.post('/', validarTokenJwt, async (req, res) => {
    ...
    // id_produtor vem do token JWT — não do body (evita fraude)
    const idProdutor = req.usuario?.id || req.usuario?.id_produtor || null;
